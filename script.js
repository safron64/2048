...
async function handleInput(key) {
	...
	if (!canMoveUp() && !canMoveDown() && !canMoveLeft() && !canMoveRight()) {
		newTile.waitForAnimationEnd().then(() => alert('Try again!'))
		return
	}
...
function moveDown() {
	slideTiles(grid.cellsGroupedByColumn.map(column => [...column].reverse()))
}
...
function canMoveDown() {
	return canMove(
		grid.cellsGroupedByColumn.map(column => [...column].reverse())
	)
}
